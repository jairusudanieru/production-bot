    ...
    async execute(interaction) {
        ...
        if (!interaction.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
            return interaction.followUp({
                content: `You don't have permission to click this button!`,
        ...
        const message = await EditorSwapManager.editSwapConfirmation(interaction.message, newProjectData, 'approve');
        if (!message) {
            await ProjectTaskReminder.editProjectTask(taskMessage, originalData);
            await ReminderManager.editProject(interaction.client, originalData);
            DatabaseManager.set(originalData.id, originalData);

            return interaction.followUp({
                content: 'Failed to approve confirmation!',
                flags: MessageFlags.Ephemeral
            });
        }
